    ...
    toHTML() {
        ...
        return `
            <li class="task">
                ...
                <label for="${this.id}" class="task-description fill-width">${this.text}</label>
...
function readStorage() {
    ...
    const input = localStorage.getItem('currentInput')
    const date = localStorage.getItem('currentDate')
    // If there are values to load
    if (!((input == '') && (date == ''))) {
        document.getElementById('enterTask').value = input;
        document.getElementById('enterDate').value  = date;
    }
